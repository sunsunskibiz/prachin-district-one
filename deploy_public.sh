...
echo -e "${GREEN}Starting PUBLIC deployment for $SERVICE_NAME to $REGION (Project: $PROJECT_ID)${NC}"
...
if command -v gcloud &> /dev/null; then
    ...
    # Deploy with override command for public_app.py
    gcloud run deploy "$SERVICE_NAME" \
        ...
        --port 8501 \
        --command "streamlit" \
        --args "run,public_app.py,--server.port=8501,--server.address=0.0.0.0"
