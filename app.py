...
def copy_to_clipboard(text):
    ...
    Note: This might be blocked by some browsers if not triggered by a direct user action,
    but it's the best attempt for an 'auto-copy' on render.
    """
    js_code = f"""
    <script>
    function copyToClipboard(text) {{
        // Try Async API first (needs HTTPS or localhost)
        if (navigator.clipboard && window.isSecureContext) {{
            navigator.clipboard.writeText(text).then(function() {{
                console.log('Async: Copy Success');
            }}, function(err) {{
                // Async failed, use Prompt
                window.prompt("Copy Google Maps Link:", text);
            }});
        }} else {{
            // No Async API, use Prompt
            window.prompt("Copy Google Maps Link:", text);
        }}
    }}
...
from utils.geo_utils import (
    create_mask_polygon,
    extract_subdistrict_name,
    extract_amphoe_name,
    process_path_overlaps,
)
from utils.data_utils import (
    ...
    load_csv_data,
    load_kml_data,
    calculate_votes_by_subdistrict,
    load_campaign_pins,
    load_subdistrict_colors,
    save_subdistrict_color,
    load_visit_records,
    save_visit_record,
    delete_visit_record,
)
...
def create_map_layers(
    ...
    if show_districts and gdf_districts is not None:
        ...
        def get_district_fill_color(row):
            ...
            if show_visit_heatmap:
                ...
                if count > 0:
                    if count < 3:
                        return [255, 204, 128, 200]  # Level 1: Light Orange
                    elif count < 7:
                        return [255, 165, 0, 200]  # Level 2: Medium Orange
                    else:
                        return [230, 81, 0, 200]  # Level 3: Dark Orange (Reddish)

            return [0, 0, 0, 0]

        gdf_districts["const_fill_color"] = gdf_districts.apply(
            get_district_fill_color, axis=1
        )
    ...
    if active_uploaded_layers:
        ...
        if layers_to_process:
            ...
            gdf_merged = process_path_overlaps(
                layers_to_process, active_uploaded_layers
            )
...
def _main_app_logic(username):
    ...
    with st.spinner("Loading data..."):
        ...
        gdf_districts = (
            gdf_districts_data.copy() if gdf_districts_data is not None else None
        )

        # Load Campaign Pins
        gdf_campaign_pins = load_campaign_pins()
        if gdf_campaign_pins is not None and not gdf_campaign_pins.empty:
            gdf_campaign_pins["tooltip_html"] = gdf_campaign_pins["name"].apply(
                lambda x: f"<b>{x}</b>"
            )
        ...
        uploaded_kml = st.sidebar.file_uploader(
            "Upload KML File (Optional)", type=["kml"], key="kml_upload_widget"
        )
            ...
            if file_id not in st.session_state.get("processed_uploads", []):
                ...
                if client:
                    ...
                    success = upload_to_gcs(
                        uploaded_kml, GCS_BUCKET_NAME, destination_path
                    )
                    if success:
                        st.sidebar.success(f"☁️ Uploaded to GCS!")
                        # Auto-Visualize
                        try:
                            gdf_new = load_kml_from_gcs(
                                GCS_BUCKET_NAME, destination_path
                            )
    ...
    selected_map_style = map_styles.get(
        map_style_selection, "mapbox://styles/mapbox/light-v9"
    )
    ...
    if gdf_districts is not None and not gdf_districts.empty:
        ...
        gdf_districts["sub_district_name"] = gdf_districts.apply(
            lambda row: extract_subdistrict_name(row, gdf_districts.columns), axis=1
        )
        gdf_districts["amphoe_name"] = gdf_districts.apply(
            lambda row: extract_amphoe_name(row, gdf_districts.columns), axis=1
        )
        ...
        gdf_districts["tooltip_html"] = gdf_districts.apply(
            get_subdistrict_tooltip, axis=1
        )
    ...
    if not df_election.empty:
        ...
        df_map_points = (
            df_election.groupby(["latitude", "longitude"])["tooltip_html"]
            .agg(aggregate_tooltips)
            .reset_index()
        )
    ...
    if st.session_state["comments"]:
        ...
        if not df_raw.empty:
            df_comments_agg = (
                df_raw.groupby(["latitude", "longitude"])
                .apply(
                    lambda x: pd.Series(
                        {"tooltip_html": create_timeline_html(x), "count": len(x)}
                    )
                )
                .reset_index()
            )
    ...
    if st.session_state["active_tab"] == "Color Assign":
        ...
        with col_map:
            ...
            layers_settings = create_map_layers(
                ...
                False,
                False,
                False,  # Hide points/comments in settings map for clarity
                show_color_orange,
                show_color_green,
                show_color_brown,
                show_color_blue,
                False,
                {},  # No Heatmap in Settings Map
                [],
                st.session_state["kml_layers"],
                None,
                None,
            ...
            # Count colors
            counts = {"orange": 0, "green": 0, "brown": 0, "blue": 0}
                    ...
                    for obj_list in obj:
                        if obj_list:
                            ...
                            if "sub_district_name" in selected_data:
                                ...
                                "properties" in selected_data
                                and "sub_district_name" in selected_data["properties"]
                            ):
                                default_sub_name = selected_data["properties"][
                                    "sub_district_name"
                                ]
                                valid_subdistrict = True
    ...
    if st.session_state["active_tab"] == "Comment Assign":
        ...
        with col_form:
                ...
                if selection and "objects" in selection and selection["objects"]:
                    ...
                    for obj_list in obj:
                        if obj_list:
                            ...
                            # Case 1: Point
                            if (
                                "latitude" in selected_data
                                and "longitude" in selected_data
                            ):
                                default_lat_c = selected_data["latitude"]
                                default_lon_c = selected_data["longitude"]
                                name_val = selected_data.get(
                                    "ชื่อหน่วยเลือกตั้ง", ""
                                ) or selected_data.get("name", "Point")
                            ...
                            # Case 2: Geometry (Polygon)
                                ...
                                if geom["type"] == "Polygon":
                                    ...
                                    sub_name = selected_data.get(
                                        "sub_district_name", ""
                                    ) or selected_data.get("properties", {}).get(
                                        "sub_district_name", "District"
                                    )
            ...
            current_loc_comments = [
                ...
                if abs(c["latitude"] - default_lat_c) < 0.0001
                and abs(c["longitude"] - default_lon_c) < 0.0001
            ]
            ...
            if current_loc_comments:
                ...
                for i, comment in enumerate(current_loc_comments):
                    ...
                    with col_txt:
                        st.caption(
                            f"{comment.get('timestamp','')}: {comment.get('text','')}"
                        )
                    with col_del:
                        if st.button(
                            "Del", key=f"del_tab_{i}_{comment.get('timestamp')}"
                        ):
    ...
    if st.session_state["active_tab"] == "Point Comment":
        ...
        with col_map_p:
            ...
            if not df_points_display.empty:
                ...
                if (
                    not current_comments.empty
                    and "target_unit" in current_comments.columns
                ):
                ...
                df_points_display[["tooltip_html", "point_color"]] = (
                    df_points_display.apply(set_props, axis=1)
                )
            ...
            layers_point = create_map_layers(
                ...
                None,
                None,  # No point comments aggregation here yet or pass generic if needed
            )
            ...
            # --- Metrics ---
            if (
                not df_points_display.empty
                and "point_color" in df_points_display.columns
            ):
                # Count Green
        ...
        with col_form_p:
            ...
            if valid_point:
                    ...
                    if submitted:
                        if c_name or c_note:
                            ...
                            if not df_map_points.empty:
                                match = df_map_points[
                                    df_map_points["tooltip_html"].str.contains(
                                        default_p_name, regex=False
                                    )
                                ]
                ...
                unit_comments = [
                    ...
                    if c.get("target_unit") == default_p_name
                ]
                ...
                if unit_comments:
                    ...
                    for i, u_comm in enumerate(unit_comments):
                        with st.container():
                            st.caption(
                                f"**{u_comm.get('contact_name')}** ({u_comm.get('timestamp')})"
                            )
    ...
    if st.session_state["active_tab"] == "Visit Record":
        ...
        with col_map_v:
            ...
            gdf_visit_display = (
                gdf_districts.copy() if gdf_districts is not None else None
            )
            if gdf_visit_display is not None:
                # Merge visit records for tooltip display
                gdf_visit_display["visit_records"] = gdf_visit_display[
                    "sub_district_name"
                ].map(visit_records)
                # Apply Visit-specific Tooltip
                gdf_visit_display["tooltip_html"] = gdf_visit_display.apply(
                    get_visit_tooltip, axis=1
                )
            ...
            layers_visit = create_map_layers(
                ...
                show_color_blue,
                show_visit_heatmap,
                visit_records,  # Pass visit data for coloring
                active_uploaded_layers,
                st.session_state["kml_layers"],
                None,
                None,
                    ...
                    for obj_list in obj:
                        if obj_list:
                            ...
                            if "sub_district_name" in selected_data:
                                ...
                                default_sub_name_v = selected_data["properties"][
                                    "sub_district_name"
                                ]
                                valid_subdistrict_v = True
            ...
            if valid_subdistrict_v:
                ...
                if existing_visits:
                    for i, v_date in enumerate(existing_visits):
                        ...
                        with c_col2:
                            if st.button(
                                "Delete", key=f"del_visit_{i}_{default_sub_name_v}"
                            ):
                                delete_visit_record(default_sub_name_v, v_date)
                                # Update local state immediately
                                if (
                                    default_sub_name_v in visit_records
                                    and v_date in visit_records[default_sub_name_v]
                                ):
                ...
                with st.form("add_visit_form"):
                    new_date = st.date_input(
                        "Select Date", value=datetime.datetime.now()
                    )
    ...
    if st.session_state["active_tab"] == "Overview":
        ...
        if not df_election.empty and "ชื่อหน่วยเลือกตั้ง" in df_election.columns:
            all_units = sorted(
                df_election["ชื่อหน่วยเลือกตั้ง"].astype(str).unique().tolist()
            )
            selected_units = st.multiselect(
                "🔍 Search Election Unit",
                options=all_units,
                placeholder="Type to search unit name...",
            )

            if selected_units:
                # Find locations of selected units
                # We do NOT filter df_election here so that aggregation picks up neighbors
                target_rows = df_election[
                    df_election["ชื่อหน่วยเลือกตั้ง"].isin(selected_units)
                ]
                filtered_locations = target_rows[
                    ["latitude", "longitude"]
                ].drop_duplicates()

                st.success(
                    f"Found {len(target_rows)} matching units. Showing all units at these locations."
                )
        ...
        if st.session_state["kml_layers"]:
            total_features = sum(
                len(gdf) for gdf in st.session_state["kml_layers"].values()
            )
            st.sidebar.success(
                f"Loaded {len(st.session_state['kml_layers'])} files ({total_features} features)"
            )
        ...
        if not df_election.empty:
            ...
            if filtered_locations is not None:
                df_map_points = df_map_points.merge(
        ...
        if df_comments_agg is not None and not df_comments_agg.empty:
            ...
            if not raw_comments.empty:
                ...
                # Filter
                mask_general = raw_comments["target_unit"].isna() | (
                    raw_comments["target_unit"] == ""
                )
                ...
                if parts:
                    ...
                    if not df_filtered_source.empty:
                        ...
                        df_comments_final = (
                            df_filtered_source.groupby(["latitude", "longitude"])
                            .apply(
                                lambda x: pd.Series(
                                    {
    ...
    if st.session_state["active_tab"] == "Analysis Details":
        ...
        if not df_election.empty:
            ...
            if not df_display.empty:
                ...
                # Show some metrics
                total_voters = (
                    df_display["ผู้มีสิทธิ์"].sum() if "ผู้มีสิทธิ์" in df_display.columns else 0
                )
                total_turnout = (
                    df_display["ผู้มาใช้สิทธิ์"].sum()
